...
window.addEventListener('load', () => {
    ...
    // Массив функций для каждого действия
    const actions = [
        () => {
        ...
        () => {
            ...
            function nextSlide(btnName, slideNames) {
                btnName.addEventListener('click', (event) => {
                    ...
                    // Для 5-го слайда
                        ...
                        // Задержка перед вызовом функции для пятого слайда
                        setTimeout(() => {
                            smoothSlideChanging(slideNames);
                            addBackstoryPopupSlideContent(
                                2500,
            ...
            // Для 4-го слайда
            nextSlide(slideBtnsOrange[3], [slides[2], slides[3]]);
    ...
    // Выполнение действий с учетом задержек
    for (const action of actions) {
        setTimeout(action, delay);
        // Увеличение задержки на 1000 миллисекунд перед следующим действием
        delay += 2000;
    }
});